...
class Subscriber(Client):
    ...
    def create_sockets(self) -> None: 
        self.subscriber = self.create_socket(zmq.XSUB, SocketCreationFunction.CONNECT, 'localhost:5557') 
        self.dealer = self.create_socket(zmq.DEALER, SocketCreationFunction.CONNECT, 'localhost:5554')
        self.dealer.setsockopt_string(zmq.IDENTITY, str(random.randint(0, 8000))) ## TODO id to be generated in __init__ or retreieved from persistent data
    ...
    def subscribe(self, topic: str) -> None:
        # TODO
        self.subscriber.send_multipart([b'\x10' + str(zmq.IDENTITY).encode('utf-8'), b'\x01' + topic.encode('utf-8')])
    ...
    def handle_msg(self) -> None: 
        ...
        [topic, msg_id, content] = MessageParser.decode(self.subscriber.recv_multipart())

        Logger.topic_message(topic, msg_id, content)
        self.dealer.send_multipart(MessageParser.encode(['ACK', topic, msg_id]))
